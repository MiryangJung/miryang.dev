...
thumbnailUrl: https://github.com/MiryangJung/rescript-remix-blog-template/raw/main/docs/home.png
tags:
  [
    'remix',
    'rescript',
...
`ReScript` 와 필요한 디펜던시를 설치한 후, `remix` 에서 설정을 몇 가지 해줘야했다.

파일을 기반으로 라우팅이 되기 때문에 `res` 파일은 읽지 않도록 ignore에 추가해준다.
...
일단 사용할 것들만 바인딩을 할 예정이여서 tsx로 먼저 파일을 만들어두고, 하나씩 진행했다.
...
이렇게 계속 하다보면 사용하는 것들을 모두 바인딩할 수 있다. <s>참 쉽죠?</s>

## 🌪 tailwind를 사용하고 싶어

style 유틸리티로 `tailwind` 를 설치한 뒤에 아무리해도 실적용이 안되어서 한참 헤맸다.  
허무하게도 tailwind를 **build하는 명령어**를 추가해주지 않아서였다.  
next.js에서는 필요없는 설정이여서 전혀 몰랐다. 🥲
...
두 번째 시련은 **css를 import하는 방법**을 모르는 것이였다.

`tsx` 에서는 아래와 같이 사용한다. res에서는 `import` 구문이 없으므로 어떻게 해야할 지 난감했다.
...
이건 도대체 `res`로 어떻게 변환할 지 감도 안와서 [forum](https://forum.rescript-lang.org/t/how-can-i-handle-mdx-in-rescript-remix/3218)에 물어보고 답을 얻었다.
...
`MDX 라우팅` 은 remix에서 제공되는 기능이여서 정말 좋았다.
...
- SSR을 어떻게 SSG처럼 사용할까?
  - remix는 SSR이다.
  - 블로그는 정적 컨텐츠이다.
  - **아마 캐시설정으로 어느정도 보완할 수 있을 듯**

## 💪🏻 끝!

아직 SEO, RSS 등의 설정이 남았지만 위의 생각들부터 해결을 해야될 듯해서 남겨두었다.  

...
블로그에 내가 생각한 문자함 말풍서 디자인으로 퍼블리싱을 하고, 배포한 workers에 연결까지 마치고 [배포](https://miryang.dev/guestbook) 를 했다.
...
코드 정리가 안되어서 Guestbook Cloudflare Workers 코드는 아직 비공개지만 정리가 되면 레포를 바로 퍼블릭으로 변경해서 공개하고 싶다.
